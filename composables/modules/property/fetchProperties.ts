...
//     const searchResults = ref([] as any) // Array to hold search results
//     const searchQuery = ref('')
//     const segmentId = ref('') // Segment ID obtained from the query parameter
//     const currentPage = ref(1)
//     const perPage = ref(10000000)
//     const totalPages = ref(1) // To store total pages
//     const sortBy = ref('all') // To store the sort type
...
//         // Pass page, perPage, and searchQuery to the API factory
//         const res = await $_fetch_properties(currentPage.value, perPage.value, searchQuery.value, route.query.tab) as any
...
//     watch(() => route.query.tab, (newSegmentId, oldSegmentId) => {
//         if (newSegmentId !== oldSegmentId) { // Ensure it's a meaningful change
//          segmentId.value = oldSegmentId || '';
//           currentPage.value = 1; // Reset to page 1 for new search
//           debouncedSearchProperties(); // Call the debounced version
//         }
//       });
...
//     onMounted(() => {
//         // Initialize segmentId from the query parameter
//         segmentId.value = route.query.tab || '';
//         getProperties();
//     })
