<!-- <template>
  <div class="relative min-h-screen max-w-2xl mx-auto flex flex-col">
    ...
    <div class="flex-grow">
      ...
      <div v-if="!uploadStep">
        <h2 class="text-lg font-medium text-[#1D2739] mb-6">Upload a clear photo of your ID card</h2>

 
        <label class="block text-sm font-medium text-gray-700 mb-2">ID Type</label>
        <div class="relative mb-4">
          <select v-model="idType"
            class="block w-full p-3 py-4 text-sm border-gray-25 rounded-lg bg-[#F0F2F5] outline-none border-[0.5px] text-gray-700">
        ...
        <div
          class="flex items-center mb-4 block w-full p-3 py-4 text-sm border-gray-25 rounded-lg bg-[#F0F2F5] outline-none border-[0.5px] text-gray-700">
          <input type="checkbox" :checked="isItemSelected"
            class="form-checkbox h-5 w-5 text-orange-500 rounded focus:ring-orange-400" disabled />
          <label class="ml-3 text-[#1D2739] text-sm font-medium">{{ getMappedId(idType) }}</label>
        </div>

   
        <div v-if="frontIdPreview?.length && backIdPreview?.length" class="flex items-center mb-6">
          <input type="checkbox" v-model="authorizationChecked"
            class="form-checkbox h-5 w-5 text-orange-500 rounded focus:ring-orange-400" />
          <label class="ml-3 text-[#1D2739] font-medium text-sm">I authorize Homeowners/Property Managers to conduct
            background checks.</label>
        </div>
          ...
          <div class="flex items-center justify-start gap-x-3">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd"
                d="M16.33 22.001H7.66C4.27 22.001 2 19.621 2 16.081V7.90998C2 4.37998 4.27 2.00098 7.66 2.00098H16.33C19.72 2.00098 22 4.37998 22 7.90998V16.081C22 19.621 19.72 22.001 16.33 22.001ZM12.01 14.94C12.48 14.94 12.87 15.331 12.87 15.81C12.87 16.3 12.48 16.69 11.99 16.69C11.51 16.69 11.12 16.3 11.12 15.81C11.12 15.331 11.51 14.94 12.01 14.94ZM11.13 8.21997C11.13 7.73997 11.52 7.34997 12.01 7.34997C12.49 7.34997 12.88 7.73997 12.88 8.21997V12.64C12.88 13.121 12.49 13.52 12.01 13.52C11.52 13.52 11.13 13.121 11.13 12.64V8.21997Z"
                fill="#4A71E0" />
            </svg>

            <p class="text-xs text-[#1D2739]">Your information is strictly used for communication with landlords or
              property managers, in line with our privacy policy.</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="uploadStep">
        ...
        <div class="my-6">
          ...
          <div
            ...
            <input type="file" class="absolute inset-0 opacity-0 cursor-pointer" @change="handleFrontIdUpload"
            ...
            <div v-else-if="frontIdPreview" class="w-full h-full flex justify-center items-center">
              ...
              <img :src="frontIdPreview" alt="Front ID Preview"
                class="rounded-lg max-w-full h-60 object-cover w-full max-h-full" />
        ...
        <div class="mb-6">
          ...
          <div
            ...
            <input type="file" class="absolute inset-0 opacity-0 cursor-pointer" @change="handleBackIdUpload"
              accept="image/*,application/pdf" />
    ...
    <div v-if="isItemSelected" class="fixed bottom-0 left-0 right-0 bg-white py-4 px-6 border-t border-gray-200">
      <div class="flex justify-between max-w-xl mx-auto">
        ...
        <button
          ...
          :disabled="!isItemSelected" @click="submitForm">
          Continue
        </button>
      </div>
    </div>
...
const firstCanProceed =  computed(() => idType.value && isItemSelected.value);
const secondCanProceed =  computed(() => frontIdPreview?.value?.length && frontIdPreview?.value?.length);
...
const submitForm = () => {
  // if(secondCanProceed.value && firstCanProceed.value) {
  //   uploadStep.value = false;
  // } 
  ...
  <div class="relative min-h-screen max-w-2xl mx-auto flex flex-col">
    ...
    <div class="flex-grow">
      ...
      <div v-if="!uploadStep">
        ...
        <div v-if="frontIdPreview && backIdPreview" class="flex items-center mb-6">
          <input type="checkbox" v-model="authorizationChecked"
            class="form-checkbox h-5 w-5 text-orange-500 rounded focus:ring-orange-400" />
          <label class="ml-3 text-[#1D2739] font-medium text-sm">I authorize Homeowners/Property Managers to conduct
            background checks.</label>
      ...
      <div v-if="uploadStep">
        ...
        <div class="my-6">
          <label class="block text-[#1D2739] mb-2 text-sm font-medium">
            Front ID <span class="text-red-500">*</span>
          </label>
          <div
            class="border border-dashed border-gray-100 bg-[#F9FAFB] p-20 w-full rounded-lg cursor-pointer flex justify-center items-center relative">
            <input type="file" class="absolute inset-0 opacity-0 cursor-pointer" @change="handleFrontIdUpload"
            ...
            <div v-else-if="frontIdPreview" class="w-full h-full flex justify-center items-center">
              ...
              <img v-if="!isFrontPdf" :src="frontIdPreview" alt="Front ID Preview"
                class="rounded-lg max-w-full h-60 object-cover w-full max-h-full" />
        ...
        <div class="mb-6">
          <label class="block text-[#1D2739] mb-2 text-sm font-medium">
            Back ID <span class="text-red-500">*</span>
          </label>
          <div
            class="border border-dashed bg-[#F9FAFB] border-gray-100 p-20 w-full rounded-lg cursor-pointer flex justify-center items-center relative">
            <input type="file" class="absolute inset-0 opacity-0 cursor-pointer" @change="handleBackIdUpload"
            ...
            <div v-else-if="backIdPreview" class="w-full h-full flex justify-center items-center">
              ...
              <img v-if="!isBackPdf" :src="backIdPreview" alt="Back ID Preview"
                class="rounded-lg max-w-full h-60 object-cover w-full max-h-full" />
    ...
    <div v-if="shouldShowFooter" class="fixed bottom-0 left-0 right-0 bg-white py-4 px-6 border-t border-gray-200">
      <div class="flex justify-between max-w-xl mx-auto">
        <button type="button"
          class="px-6 py-3 text-sm text-gray-600 bg-gray-100 rounded-md hover:bg-gray-200 focus:outline-none"
          @click="goBack">
        ...
        <button
          class="px-6 py-3 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50 bg-black rounded-md hover:bg-gray-800 focus:outline-none transition-opacity"
          :disabled="!canProceed" @click="submitForm">
          Continue
        </button>
      </div>
    </div>
...
const canProceed = computed(() => {
  if (!uploadStep.value) {
    // First step: ID type and authorization must be checked (if both IDs are uploaded)
    if (frontIdPreview.value && backIdPreview.value) {
      return !!idType.value && isItemSelected.value && authorizationChecked.value;
    }
    // If IDs not uploaded yet, just check ID type is selected
    return !!idType.value && isItemSelected.value;
  } else {
    // Second step (upload step): Both front and back ID must be uploaded
    return !!(
      idType.value && 
      frontIdPreview.value && 
      backIdPreview.value
    );
  }
});

// Computed property to show footer only when appropriate
const shouldShowFooter = computed(() => {
  if (!uploadStep.value) {
    // Show footer on first step if ID type is selected
    return isItemSelected.value;
  } else {
    // Always show footer on upload step
    return true;
  }
});
...
// Navigate to the upload page or submit
const submitForm = () => {
  if (!canProceed.value) return;

  if (!uploadStep.value) {
    // If on first step and both IDs are already uploaded, validate authorization
    if (frontIdPreview.value && backIdPreview.value) {
      if (!authorizationChecked.value) {
        showToast({
          title: "Authorization Required",
          message: "Please authorize background checks to continue",
          toastType: "error",
          duration: 3000,
        });
        return;
      ...
      // Both IDs uploaded and authorized, proceed to final step
      const formData = {
        type: idType.value,
        fileUrls: [...documentData.value.fileUrls]
      };
    ...
    // Validate that both IDs are uploaded before proceeding
    if (!frontIdPreview.value || !backIdPreview.value) {
      showToast({
        title: "Missing Information",
        message: "Please upload both front and back of your ID card",
        toastType: "error",
        duration: 3000,
      });
      return;
    ...
    // Go back to first step to show authorization checkbox
    uploadStep.value = false;
  }
};
...
const handleFrontIdUpload = async (event: Event) => {
  ...
  if (input.files && input.files[0]) {
    ...
    try {
      ...
      if (uploadResponse.value) {
        frontIdPreview.value = uploadResponse.value.url;
        isFrontImage.value = uploadResponse.value.type?.startsWith('image') || false;
        isFrontPdf.value = uploadResponse.value.type?.startsWith('application/pdf') || false;
      ...
      showToast({
        title: "Upload Failed",
        message: "Failed to upload front ID. Please try again.",
        toastType: "error",
        duration: 3000,
      });
    } finally {
...
const handleBackIdUpload = async (event: Event) => {
  ...
  if (input.files && input.files[0]) {
    ...
    try {
      ...
      if (uploadResponse.value) {
        backIdPreview.value = uploadResponse.value.url;
        isBackImage.value = uploadResponse.value.type?.startsWith('image') || false;
        isBackPdf.value = uploadResponse.value.type?.startsWith('application/pdf') || false;
      ...
      showToast({
        title: "Upload Failed",
        message: "Failed to upload back ID. Please try again.",
        toastType: "error",
        duration: 3000,
      });
    } finally {
