    ...
    >
        ...
        <div class="space-y-4">
          <div v-for="question in questions" :key="question.id" class="mb-4">
            ...
            <template v-else-if="question.widgetType === 'long-response'">
              <textarea
                v-model="question.answer"
                class="p-3 py-3.5 border-[0.5px] outline-none text-sm bg-[#E4E7EC] rounded-lg w-full min-h-[100px]"
                :placeholder="question.placeholder || 'Enter your response'"
              ></textarea>
            </template>
            <template v-else>
      ...
      >
        <div class="max-w-2xl w-full flex justify-between">
          ...
          <button
            class="px-6 py-3 text-sm rounded-md bg-[#292929] text-white disabled:opacity-50 disabled:cursor-not-allowed"
...
// Computed property to check if all required questions are answered
const areAllRequiredQuestionsAnswered = computed(() => {
  if (!questions.value.length) return false;
  
  // Check if all questions have answers (treating all as required if no 'required' field exists)
  return questions.value.every((question: any) => {
    // If there's a required field, check only required questions
    ...
    // For other types (numbers, booleans, etc.), just check truthy
    return true;
  });
});
    ...
    if (newVal && newVal.preScreeningQuestions) {
      ...
      questions.value = newVal?.preScreeningQuestions.map((q: any) => {
        ...
        return {
          ...
          question: q.question,
          answer: savedAnswer ? savedAnswer?.answer : q?.answer || "", // Use saved answer if available, else fallback to default
          required: q.required !== undefined ? q.required : true, // Default to required if not specified
        };
      });
    }
  },
...
// Save the answers and go to the next step
const goNext = () => {
  if (!areAllRequiredQuestionsAnswered.value) return;
