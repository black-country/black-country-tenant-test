<template>
    <div class="min-h-screen flex justify-center items-center bg-white">
      <form @submit.prevent="verifyMoveIn" class="w-full max-w-md text-center">
       <div class="mb-3 flex justify-center items-center">
        ...
        <h1 class="text-2xl font-semibold mb-4 text-[#1D2739]">Please Enter OTP</h1>
        <p class="text-[#667185] mb-8">
            Kindly enter the OTP sent to {{  user?.email ?? 'Nil' }} to proceed to app move-in
        </p>
      <section>
     ...
     <div class="pt-10">
      <button :disabled="loading || !isOtpComplete" type="submit" class="bg-[#292929] disabled:cursor-not-allowed disabled:opacity-25 text-white py-4 px-6 rounded-lg w-full mb-4">
        {{  loading ? 'processing..' : 'Verify email' }}
      </button>
     </div>
        <p class="text-gray-500">
          <button :disabled="processing" @click="intiateMoveIn" class="text-[#667185] disabled:cursor-not-allowed disabled:opacity-25 ">{{ processing ? 'processing...' : 'Resend code'}}</button>
        </p>
      </form>
    </div>
  </template>
  ...
  const { loading,
    setCredential,
    credential,
    verifyMoveIn } = useVerifyMoveInOTP()
  ...
  const focusNext = (index: number) => {
    ...
    credential.value.otp = otp.value.join('')
  }
