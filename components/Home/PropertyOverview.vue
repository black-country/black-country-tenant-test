<!-- components/PropertyOverview.vue -->
<template>
    <div class="relative">
      <img :src="myHomeInfo?.house?.images[0]" alt="Property Image" class="w-full h-[500px] object-cover rounded-lg" />
     <div class="">
      <div class="absolute bottom-0 bg-black bg-opacity-70 space-y-3 w-full text-white px-4 py-6 rounded-lg">
        <h3 class="font-semibold">{{ myHomeInfo?.house?.name ?? 'Nil' }}: {{ myHomeInfo?.room?.name ?? 'Nil' }}</h3>
        <p class="flex items-center gap-x-2 text-sm">
          ...
          {{ myHomeInfo?.house?.address ?? 'Nil' }}</p>
      </div>
     </div>
    </div>
  </template>
