  ...
  <div v-if="isWelcomeModalActive && !isTourCompleted('home-page')" class="modal-overlay">
    <div class="modal">
      ...
      <p class="text-[#667185] mb-6 leading-snug text-[15px]">
        ...
        <span class="text-[#1D2739] font-semibold">'Continue'</span> to embark on your personalized <span
          class="font-semibold text-[#1D2739]">tour</span> of the platform!
      </p>
      <div class="modal-footer  flex justify-between space-x-4 pt-5">
        <button @click="handleAction('skip')"
          class="flex-1 bg-[#EBE5E0] font-medium text-[#292929] py-3.5 px-4 rounded-lg hover:bg-gray-200">Skip
          Tour</button>
        <button @click="handleAction('continue')"
          class="flex-1 bg-[#292929] text-white py-3.5 px-4 rounded-lg hover:bg-gray-900">Continue</button>
      </div>
    </div>
  </div>
</template>
...
const { isWelcomeModalActive, isTourCompleted, handleWelcomeModalAction } = useTourGuide();
