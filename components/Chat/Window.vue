<!-- <template>
  ...
  // Format date headers based on message timestamp
...
// Determine if a new date header should be shown
const shouldDisplayDateHeader = (index: number): boolean => {
  if (index === 0) return true;

  const currentMessageDate = moment(props.roomChats[index].createdAt);
  const previousMessageDate = moment(props.roomChats[index - 1].createdAt);
