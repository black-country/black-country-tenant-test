  ...
  <div class="flex items-center px-4 py-3 border-b-[0.5px] border-gray-100 w-full">
    <div class="flex items-center">
      ...
      <svg v-else class="h-10 w-10" viewBox="0 0 111 106" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <h3 v-if="selectedUser?.participant?.id" class="ml-1 font-semibold text-lg">{{selectedUser?.participant?.firstName}} {{selectedUser?.participant?.lastName}}</h3>
        <h3 v-else class="ml-1 font-semibold text-lg">Deleted User</h3>
    </div>
    <div class="ml-auto flex items-center space-x-4">
      <button @click="copyToClipboard(selectedUser?.participant?.firstName)" class="p-2 rounded-full transition-colors">
