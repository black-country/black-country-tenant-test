...
const placeMarkers = (map: google.maps.Map) => {
  ...
  displayedProperties.value.forEach((property) => {
    ...
    popupContent.className =
      "relative cursor-pointer min-w-[300px] w-full lg:max-w-[350px] bg-white";
    popupContent.innerHTML = `
  <img class="rounded-lg h-44 w-full object-cover" src="${
    property.images[0] || ""
  }" alt="${property.name}" />
  <section class="space-y-2">
    <div>
    ...
    } bedrooms | ${property.bathroomCount} baths | ${property.sizes} ${
      property.sizeUnit
    }</p>
    <p class="text-[#1D2739] text-xs">${
      property.availableRoomsCount
    } rooms avail. <span class="font-medium text-[#326543]">Now</span> | ${
      property.unavailableRoomsCount
    } room avail. on ${moment(property.createdAt).format("MMMM Do YYYY")}</p>
  </section>
`;

    popupContent.addEventListener("click", () => {
      window.location.href = `/dashboard/listings/${property.id}/preview`; // Adjust the URL as per your routing structure
    });
