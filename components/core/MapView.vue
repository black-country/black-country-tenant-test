...
const { propertiesList, loadingProperties, searchQuery } = useGetProperties();
const { saveSearch, loading: saveLoading, setPayload } = useSaveSearch();

// Computed property for filtered properties
const filteredProperties = computed(() => {
  if (!searchQuery.value) return propertiesList.value;

  return propertiesList.value?.filter((property) =>
...
const createInfoWindowContent = (property: Property) => {
  return `
    <div class="max-w-sm">
      ...
      <p class="text-[#1D2739] text-xs">${property?.available || 'Available Now'}</p>
...
// Watch for changes in saveLoading
watch(saveLoading, (newValue) => {
  if (activeInfoWindow.value && activeProperty.value) {
    // Update the info window content to reflect the new loading state
    activeInfoWindow.value.setContent(createInfoWindowContent(activeProperty.value));
  }
});
