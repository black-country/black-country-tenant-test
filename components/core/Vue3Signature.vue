    ...
    <div class="bg-white rounded-lg mt-3">
      <section class="flex justify-between items-center mb-3">
        ...
        <div v-if="mode === 'draw' && !savedSignature" class="mb-4 border-[0.5px] rounded-lg">
          ...
          <div v-if="savedSignature" class="flex flex-col items-center border-[0.5px] border-gray-300 rounded-lg">
              ...
              />
              <button @click="resetSignature" class="bg-black text-white rounded-lg text-xs mb-3 px-4 py-2 rounded-sm">
                Sign Again
              </button>
          </div>
  ...
  <div class="bg-white rounded-lg mt-3">
    ...
    <div v-if="mode === 'draw' && !savedSignature" class="mb-4 border-[0.5px] rounded-lg">
      <Vue3Signature
        ...
        :h="'200px'"
      />
      <div class="flex justify-end space-x-4 mt-2 p-4 sm:p-6 lg:p-4">
        <button @click="saveSignature('image/jpeg')" class="bg-black text-white text-xs px-4 py-2 rounded-sm">
          Save Signature
        </button>
        <button @click="clearSignature" class="bg-black text-white text-xs px-4 py-2 rounded-sm">
          Clear
    ...
    <div v-if="mode === 'type' && !savedSignature" class="mb-4 border-[0.5px] rounded-lg">
      ...
      <div class="flex justify-end m-2">
        <button @click="saveTypedSignatureAsImage" class="bg-black text-white text-xs px-3 py-2.5 rounded-md">
          Save Signature
        </button>
      </div>
    </div>
    ...
    <div v-if="mode === 'upload' && !savedSignature" class="mb-4 border-[0.5px] rounded-lg">
      <div>
        ...
        <div v-if="image" class="mb-4">
          <img :src="image" class="w-full h-[400px] object-contain" />
          <div class="flex justify-center space-x-4 my-4">
            <button 
              @click="$refs.fileInput.click()" 
              class="bg-gray-200 text-black text-xs px-3 py-2 rounded-md"
...
const saveUploadedSignature = async () => {
  if (image.value) {
    savedSignature.value = image.value
    emit('signature', savedSignature.value) 

    const file = new File([image.value], 'uploaded-signature.png')
    await uploadSignatureFile(file)
  }
}
    ...
    <div v-if="!loading">
      <div v-if="savedSignature" class="mt-6 p-4 rounded-lg border border-gray-50 bg-[#F0F2F5]">
        ...
        <div v-if="mode === 'draw' || mode === 'upload'" class="f">
          <img :src="savedSignature" alt="Signature" class="border p-2 h-32 w-full object-cover rounded" />
        </div>
