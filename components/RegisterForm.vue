<template>
  <div class="max-w-lg p-3 mx-auto bg-white lg:p-8">
    ...
    <!-- Form Header -->
    <div class="flex flex-col items-center justify-center pb-4 mb-6 text-center gap-y-3">
    ...
    <form @submit.prevent="handleRegister">
      <div class="mb-4">
        ...
        <input type="text" id="full-name" v-model="credential.fullName.value" placeholder="Enter your full name"
      ...
      <div class="relative mb-4">
        ...
        <input type="email" id="email" v-model.trim="payload.email.value" placeholder="Enter your email address"
          class="mt-1 block w-full bg-[#E4E7EC] text-sm px-4 py-4 border-[0.5px] border-gray-100 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
        <!-- <input
          type="email"
          id="email"
        ...
        /> -->
        <!-- Loading Spinner -->
        <div v-if="processing" class="absolute inset-y-0 flex items-center right-4 top-6">
          <svg class="w-5 h-5 text-gray-900 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
            viewBox="0 0 24 24">
        ...
        <p v-if="emailExists" class="mt-2 text-xs text-red-500">
      ...
      <div class="relative mb-4">
        ...
        <input :type="showPassword ? 'text' : 'password'" id="password" min="6" v-model.trim="credential.password.value"
          placeholder="Enter your password"
          class="mt-1 block w-full bg-[#E4E7EC] text-sm px-4 py-4 border-[0.5px] border-gray-100 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
        <div @click="toggleShowPassword" class="absolute inset-y-0 flex items-center cursor-pointer right-4 top-6">
          <svg v-if="!showPassword" width="20" height="20" viewBox="0 0 20 20" fill="none"
            ...
            <path
              ...
              stroke="#1D2739" stroke-width="2" stroke-linecap="round" />
            <path d="M12.5 11.25L13.75 13.3333" stroke="#1D2739" stroke-width="1.5" stroke-linecap="round"
              stroke-linejoin="round" />
            <path d="M16.6665 9.16669L18.3332 10.8334" stroke="#1D2739" stroke-width="1.5" stroke-linecap="round"
              stroke-linejoin="round" />
            <path d="M1.6665 10.8334L3.33317 9.16669" stroke="#1D2739" stroke-width="1.5" stroke-linecap="round"
              stroke-linejoin="round" />
            <path d="M7.5 11.25L6.25 13.3333" stroke="#1D2739" stroke-width="1.5" stroke-linecap="round"
              stroke-linejoin="round" />
          </svg>
        ...
        <p v-if="checkPasswordLength" class="mt-2 text-xs text-red-500">
      ...
      <div class="relative mb-4">
        ...
        <input :type="showConfirmPassword ? 'text' : 'password'" id="confirm-password" min="6"
          v-model.trim="credential.confirmPassword.value" placeholder="Confirm your password"
          class="mt-1 block w-full bg-[#E4E7EC] text-sm px-4 py-4 border-[0.5px] border-gray-100 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
        <div @click="toggleConfirmShowPassword"
          class="absolute inset-y-0 flex items-center cursor-pointer right-4 top-6">
        ...
        <p v-if="passwordMismatch && hasTypedConfirmPassword" class="mt-2 text-xs text-red-500">
          Passwords do not match.
        </p>
      </div>

      <div class="flex items-center mb-4">
        <input id="terms" type="checkbox" v-model="credential.agreement.value"
      ...
      <button type="submit" :disabled="isFormDisabled || processing || emailExists"
    ...
    <p class="mt-6 text-center text-gray-600">
      Have an account?
      <NuxtLink to="/login" class="text-[#326543] font-semibold">Login</NuxtLink>
    </p>
  </div>
...
const checkPasswordLength = computed(() => {
  return credential.password.value.length > 0 && credential.password.value.length < 6;
});
