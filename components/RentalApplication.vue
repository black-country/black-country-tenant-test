...
<template>
   <main>
     <!-- <div v-if="!loading && Object.keys(propertyObj?.rentalApplication).length" class="max-w-3xl mx-auto p-4"> -->
      ...
      <!-- Progress Steps -->
      <ProgressSteps :propertyObj="propertyObj" />
  
      <!-- Property Details -->
      <div class="mb-6 mt-16 ">
        <div v-if="propertyObj?.images" class="flex items-center gap-4">
          <img :src="propertyObj?.images[0]" alt="Room preview" class="w-20 h-20 rounded-lg rounded-full object-cover" />
          <div>
            <h2 class="text-lg font-semibold">{{ propertyObj?.name }}: {{ propertyObj.rentalApplication?.room?.name }}</h2>
            <NuxtLink :to="`/dashboard/listings/${propertyObj?.id}/preview`" class="text-blue-600 hover:underline text-sm">View property details</NuxtLink>
          </div>
        </div>
      </div>
      ...
      <div class="bg-[#E8EDFB] p-4 rounded-lg mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">
          Your application is currently been reviewed. When it has been approved and a lease agreement has been sent, the sign agreement button below will be enabled for you to proceed with the rental process
        </p>
      </div>
      ...
      <div class="flex border-[0.5px] py-4 px-3 rounded-lg border-gray-100 justify-between items-center mb-6">
        ...
        <div>
          <span class="text-gray-900 font-medium">{{ formatCurrency(propertyObj?.rentalApplication?.room?.rentAmount) ?? '0.00' }}</span>
          <span class="text-gray-600 ml-2">{{propertyObj?.rentalApplication?.room?.rentFrequency ?? 'Nil'}}</span>
        </div>
      </div>
      ...
      <div class="mb-6 border-[0.5px] py-4 px-3 rounded-lg border-gray-100 flex justify-between items-center">
        ...
        <div v-if="propertyObj.rentalApplication?.room?.isMaster" class="inline-flex items-center gap-2 px-3 py-1 bg-[#FEF6E7] text-xs text-[#DD900D] rounded-full mt-2">
        ...
        <button class="w-full flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div class="flex justify-start items-start flex-col">
              ...
              <NuxtLink :to="`/dashboard/listings/${propertyObj?.id}/room-interior-images`" class="text-sm text-[#2E5C3D] underlined">Click to view photos of all common areas</NuxtLink>
            </div>
          </div>
      ...
      <div class="p-4">
          ...
          <div class="mb-4 border-[0.5px] border-gray-50 py-4 rounded-lg px-3">
            ...
            <span class="ml-2">{{ propertyObj?.rentalApplication?.room?.isFurnished ? 'Yes' : 'No' }}</span>
          </div>
      ...
      <!-- <div class="mb-6">
        ...
        <h4 class="text-gray-600 mb-3 text-sm">Amenities</h4>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div v-for="(amenity, idx) in propertyObj?.commonAreas" :key="idx" class="flex items-center gap-2 p-3.5 text-sm border-gray-100 border-[0.5px] rounded-lg">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g opacity="0.8">
        ...
        <button class="text-[#2E5C3D] underline mt-3">View more</button>
      </div> -->
  
      <!-- Action Buttons -->
      <div class="flex gap-4 mt-8">
        <button :disabled="isCancelApplicationDisabled"
        @click="showCancelModal = true" class="flex-1 flex disabled:cursor-not-allowed disabled:opacity-25 justify-center items-center flex-col gap-y-3 text-sm py-3 px-4  text-red-600 rounded-lg">
...
const displayedCommonAreas = computed(() => {
  return showMore.value ? propertyObj?.value?.commonAreas : propertyObj?.value?.commonAreas.slice(0, 6);
});
