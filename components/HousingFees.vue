        ...
        <div v-for="fee in fees" :key="fee.name"
            ...
            <div class="flex items-center justify-between space-x-2 w-full">
               <div class="space-y-2">
                ...
                <div>
                    <button v-if="fee.description" @click="openModal(fee)" class="text-blue-500">
        ...
        <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-[#F9FAFB] rounded-lg shadow-lg max-w-md w-full pb-6">
                ...
                <div class="flex justify-between items-center bg-white p-4 rounded-t-xl">
                    <h3 class="text-lg font-medium text-[#1D2739] mb-2">{{ selectedFee?.name }}</h3>
                ...
                <div class="bg-white p-4 m-6 rounded-lg">
                    <p class="text-gray-700 mb-4 text-sm leading-snug">{{ selectedFee?.description }}</p>
                 <div class="flex justify-between items-center tetx-sm">
                    <p class="text-[#667185]">Amoount</p>
                    <p class="text-[#475367]"><span class="text-[#1D2739]">{{ selectedFee?.amount }}</span> {{ selectedFee?.frequency }}</p>
                 </div>
                </div>
            </div>
        </div>
