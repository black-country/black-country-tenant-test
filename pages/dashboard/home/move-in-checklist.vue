<template>
  <main>
    <TopNavBar />
    <div class="flex flex-col justify-between max-w-3xl min-h-screen mx-auto">
      <div class="p-6 bg-white">
        ...
        <div v-if="Object.keys(checklist)?.length && !loading" class="flex items-center justify-center">
          <HomeMoveChecklist :loading="loading" :checklist="checklist" actionType="move-in"
            @update-state="updateChecklistState" :rentalChecklist="rentalChecklist" />
        </div>
        <CoreFullScreenLoader v-else-if="loading && !Object.keys(checklist).length" :visible="loading"
          text="Fetching checklist...." logo="" />
        <section v-else
          class="flex flex-col justify-between border-[0.5px] border-gray-100 rounded-lg py-10 items-center space-y-2 mt-10">
    ...
    <div v-if="Object.keys(checklist)?.length" class="flex items-center justify-between max-w-3xl px-4 py-10 mx-auto">
      <button @click="router.back()"
        class="rouned-lg text-sm px-6 py-3 text-[#292929] rounded-lg border-[0.5px] border-gray-100">
        Cancel
      </button>
      <button @click="submitChecklist" :disabled="processing"
...
const submitChecklist = async () => {
  try {
    ...
    showToast({
      title: "Success",
      message: 'Checklist successfully submitted!',
      toastType: "success",
      duration: 3000
    });
  } catch (error) {
