      ...
      <div class="bg-white p-6">
        ...
        <div class="flex items-center text-gray-600 mb-4">
          ...
          <span class="ml-2 text-[#667185]"
            >My Home <span class="ml-2 text-[#667185]"> | Move Out</span> |
            <span class="text-[#1D2739] font-medium">Move - out checklist</span>
          </span>
        </div>
        <div
          v-if="Object.keys(checklist)?.length && !loading"
...
import { ref } from "vue";
import { useAddMoveOutChecklistItem } from "@/composables/modules/home/useAddMoveOutChecklist";
import { useFetchMoveOutRentalChecklist } from "@/composables/modules/home/useFetchMoveoutRentalChecklist";
...
const {
  ...
  addChecklistItem,
  setPayload,
} = useAddMoveOutChecklistItem();
const { loading: fetchingRentalsChecklist, rentalChecklist } =
  useFetchMoveOutRentalChecklist();
...
const submitChecklist = async () => {
  try {
    ...
    if (
      ...
      !Array.isArray(checklistState.value) ||
      !Array.isArray(checklistState.value[0])
    ) {
      console.error(
        "Invalid checklist state. Ensure it is an array of arrays."
      );
      return;
    }
