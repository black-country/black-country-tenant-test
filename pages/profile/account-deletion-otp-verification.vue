<template>
<main>
    <div v-if="route.query.step === 'enter-otp'" class="min-h-screen flex justify-center flex-col items-center bg-white">
      <section>
        <div @submit.prevent="validateDeleteAccountOTP" class="w-full max-w-md text-center">
            ...
            <section>
                <div class="flex justify-center space-x-2 mb-4">
                    <input placeholder="0" :class="[errorMessage ? 'border-[#9E0A05] border-2' : '']" v-model="otp[0]"
                        @input="focusNext(0)" type="text" maxlength="1"
                        class="w-16 outline-none focus-within:border-2 focus-within:border-[#5B8469] h-14 text-center border rounded-lg text-2xl" />
                    <input placeholder="0" :class="[errorMessage ? 'border-[#9E0A05] border-2' : '']" v-model="otp[1]"
                        @input="focusNext(1)" type="text" maxlength="1"
                        class="w-16 outline-none focus-within:border-2 focus-within:border-[#5B8469] h-14 text-center border rounded-lg text-2xl" />
                    <input placeholder="0" :class="[errorMessage ? 'border-[#9E0A05] border-2' : '']" v-model="otp[2]"
                        @input="focusNext(2)" type="text" maxlength="1"
                        class="w-16 outline-none focus-within:border-2 focus-within:border-[#5B8469] h-14 text-center border rounded-lg text-2xl" />
                    <input placeholder="0" :class="[errorMessage ? 'border-[#9E0A05] border-2' : '']" v-model="otp[3]"
                        @input="focusNext(3)" type="text" maxlength="1"
                        class="w-16 outline-none focus-within:border-2 focus-within:border-[#5B8469] h-14 text-center border rounded-lg text-2xl" />
                </div>
                <p v-if="errorMessage" class="text-sm text-[#1D2739]">The entered OTP code is incorrect. Please verify
                    or click 'Resend code' for a new one.</p>
            </section>
            <div class="pt-10">
                <!-- <button :disabled="loading || !isOtpComplete" type="submit"
                    class="bg-[#292929] disabled:cursor-not-allowed disabled:opacity-25 text-white py-4 px-6 rounded-lg w-full mb-4">
                    {{ loading ? 'processing..' : 'Verify email' }}
                </button> -->
                <button @click="handleNextStep" :disabled="loading || !isOtpComplete" type="button"
                    class="bg-[#292929] disabled:cursor-not-allowed disabled:opacity-25 text-white py-4 px-6 rounded-lg w-full mb-4">
                    Continue
                </button>
            </div>
        ...
        <p class="text-gray-500 flex justify-center items-center">
            <button @click="handleResendOtp" :disabled="initiating" type="button" class="text-[#667185] disabled:cursor-not-allowed disabled:opacity-25">{{ initiating ? 'processing...' : 'Resend code'}}</button>
        </p>
      </section>
    </div>
    <DeleteAccountReasons :loading="loading" v-if="route.query.step === 'deletion-reason'" @proceedToDeleteAccount="handleProceedToDelete" />
</main>
</template>
...
import { useValidateDeleteAccountOTP } from '@/composables/modules/settings/useValidateDeleteAccount'
import DeleteAccountReasons from '../../components/DeleteAccountReasons.vue'

const { validateDeleteAccountOTP, loading, payload, setPayloadObj } = useValidateDeleteAccountOTP()
const { initiateDeleteAccount, loading: initiating, setPayload } = useInitiateDeleteAccount()
...
const accountDeletionStep = ref('enter-otp')
...
const focusNext = (index: number) => {
    ...
    payload.otpCode.value = otp.value.join('')
}
...
watch(otp, (newOtp: any) => {
    ...
    if (newOtp.every((digit: any) => digit.length === 1)) {
        ...
        // const otpString = otp.value.join(''); // Convert OTP array to string
        // const payload = {
        ...
        // validateDeleteAccountOTP().catch((error: any) => {
...
const handleResendOtp = () => {
    ...
    console.log(payloadObjData, 'payloaf hee')
    setPayload(payloadObjData)
    initiateDeleteAccount().then(() => {
        otp.value = ['', '', '', ''] // Clear the OTP array after resending code
        // Reset error messages or states if necessary
    })
}
...
const { validateDeleteAccountOTP, loading, setPayloadObj } = useValidateDeleteAccountOTP();
const { initiateDeleteAccount, loading: initiating, setPayload } = useInitiateDeleteAccount();
const route = useRoute();
const router = useRouter();
...
onMounted(() => {
    // accountDeletionStep.value = "enter-otp";
    const newQueryParams = {
        ...route.query, // Preserve existing query parameters if needed
        step: "enter-otp", // Add or modify the `step` query parameter
    };

    router.replace({ query: newQueryParams }); // Use replace to avoid adding to the history stack
});
...
const handleNextStep = () => {
  ...
  const newQueryParams = {
    ...
    step: "deletion-reason", // Add or modify the `step` query parameter
  };

  router.push({ query: newQueryParams }); // Update the query parameters
}
...
const isOtpComplete = computed(() => otp.value.every((digit) => digit.length === 1));

const validateOtpAndProceed = async () => {
    try {
        const otpCode = otp.value.join("");
        setPayloadObj({ otpCode, email: route.query.email });
        await validateDeleteAccountOTP();
        // accountDeletionStep.value = "delete-reason"; // Switch to DeleteAccountReasons
    } catch (error) {
        errorMessage.value = true; // Show error if OTP validation fails
    }
};

const handleResendOtp = async () => {
    try {
        setPayload({ email: route.query.email });
        await initiateDeleteAccount();
...
const handleProceedToDelete = async (selectedReason: string) => {
    const otpCode = otp.value.join("");
    const payload = {
        email: route.query.email,
        otpCode,
    ...
    // Send payload to the API to complete account deletion
    console.log("Proceeding to delete account with payload:", payload);
};
</script>
