<template>
  <main>
    <TopNavBar />
    <div v-if="!loading" class="w-full max-w-2xl p-3 mx-auto bg-white lg:p-6">
      ...
      <form @submit.prevent="handleSave" class="mt-6 space-y-4">
        <div>
          ...
          <input v-model="credential.currentLandlord" type="text"
        ...
        <div>
          ...
          <input v-model="credential.rentalAddress" type="text"
            class="w-full p-2 mt-1 outline-none focus-within:border-2 focus-within:border-[#5B8469] border-[0.5px] text-sm rounded-md bg-[#E4E7EC] py-4"
        ...
        <div>
          ...
          <input v-model="credential.lengthOfTenancy" type="tel"
            class="w-full p-2 mt-1 outline-none focus-within:border-2 focus-within:border-[#5B8469] border-[0.5px] text-sm rounded-md bg-[#E4E7EC] py-4"
            placeholder="Enter length of tenancy" />
        </div>

        <div>
          <label class="text-[#1D2739] text-sm">Reason for Moving Out <span class="text-red-500">*</span></label>
          <textarea v-model="credential.reasonForMovingOut" rows="10" cols="10"
            ...
            placeholder="Enter reason for moving out"></textarea>
        </div>
      </form>

      <div class="flex justify-between w-full max-w-2xl mx-auto mt-6 bg-white lg:p-4">
        <button @click="router.back()"
          class="text-[#292929] border rounded-md px-4 py-3 hover:bg-gray-100">Cancel</button>
        <button :disabled="updating || !isFormValid" @click="handleSave"
          class="text-white disabled:opacity-25 disabled:cursor-not-allowed font-medium rounded-md px-6 py-3 bg-[#292929]">{{
            updating ? 'Processing..' : 'Save' }}</button>
      </div>
    </div>
    <div v-else class="max-w-4xl p-4 mx-auto mt-10 rounded-md">
...
const isFormValid = computed(() => {
  ...
  return (
    c.currentLandlord?.trim() &&
    c.rentalAddress?.trim() &&
    c.lengthOfTenancy?.trim() &&
    c.reasonForMovingOut?.trim()
  );
});
