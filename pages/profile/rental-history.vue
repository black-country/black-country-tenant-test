      ...
      <div v-if="!loading" class="max-w-2xl mx-auto bg-white p-3 lg:p-6 w-full">
        ...
        <form @submit.prevent="handleSave" class="mt-6 space-y-4">
          <div>
            <label class="text-[#1D2739] text-sm">Current Landlord's Name <span class="text-red-500">*</span></label>
            <input 
              v-model.trim="credential.currentLandlord" 
              type="text" 
          ...
          <div>
            <label class="text-[#1D2739] text-sm">Rental Address <span class="text-red-500">*</span></label>
            <input 
              v-model.trim="credential.rentalAddress" 
              type="text" 
              class="w-full p-2 mt-1 outline-none focus-within:border-2 focus-within:border-[#5B8469] border-[0.5px] text-sm rounded-md bg-[#E4E7EC] py-4" 
          ...
          <div>
            <label class="text-[#1D2739] text-sm">Length of Tenancy <span class="text-red-500">*</span></label>
            <input 
              v-model.trim="credential.lengthOfTenancy" 
              type="text" 
              class="w-full p-2 mt-1 outline-none focus-within:border-2 focus-within:border-[#5B8469] border-[0.5px] text-sm rounded-md bg-[#E4E7EC] py-4" 
              placeholder="Enter length of tenancy (e.g., 2 years)"
              required 
            />
          </div>
          ...
          <div>
            <label class="text-[#1D2739] text-sm">Reason for Moving Out <span class="text-red-500">*</span></label>
            <textarea 
              v-model.trim="credential.reasonForMovingOut" 
              rows="10" 
              cols="10" 
              class="w-full resize-none p-2 mt-1 outline-none focus-within:border-2 focus-within:border-[#5B8469] border-[0.5px] text-sm rounded-md bg-[#E4E7EC] py-4" 
              placeholder="Enter reason for moving out"
              required
            ></textarea>
          </div>
        </form>
        ...
        <div class="flex justify-between lg:p-4 bg-white mt-6 max-w-2xl mx-auto w-full">
          <button 
            @click="router.back()" 
            type="button"
            class="text-[#292929] border rounded-md px-4 py-3 hover:bg-gray-100 transition-colors"
          >
            Cancel
          </button>
          <button 
            ...
            @click="handleSave" 
            class="text-white disabled:opacity-50 disabled:cursor-not-allowed font-medium rounded-md px-6 py-3 bg-[#292929] hover:bg-gray-800 transition-colors"
          >
...
// Computed property to check if all required fields are filled
const isFormValid = computed(() => {
  // Check if all fields exist and have non-empty trimmed values
  return !!(
    credential.value.currentLandlord?.trim() &&
    credential.value.rentalAddress?.trim() &&
    credential.value.lengthOfTenancy?.trim() &&
    credential.value.reasonForMovingOut?.trim()
  );
});
...
const handleSave = async () => {
  ...
  if (!isFormValid.value || updating.value) {
    return;
  }

  await updateProfile({
    currentLandlord: credential.value.currentLandlord.trim(),
    rentalAddress: credential.value.rentalAddress.trim(),
    lengthOfTenancy: credential.value.lengthOfTenancy.trim(),
    reasonForMovingOut: credential.value.reasonForMovingOut.trim()
  });
